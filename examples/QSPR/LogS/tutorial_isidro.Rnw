...
\author[2]{\rm Isidro Cortes-Ciriano\thanks{isidrolauscher@gmail.com}} 
...
setwd('/Users/icortes/Desktop/eve/eve/examples/QSPR/LogS')
@
...
get_error_model <- function(model,dataset,method="svm",tune.grid=tune.grid){
  ...
  error_model <- train(dataset$x.train, abs(predObsCV$pred - predObsCV$obs), method, 
               tuneGrid=tune.grid, trControl=dataset$trControl,
               savePredictions=T)
  return(error_model)
}

error_model <- get_error_model(model, dataset, method="svmRadial", tune.grid=tune.grid) 

saveRDS(error_model, file=paste(method,"_error_model.rds",sep=""))
...
confi_intervals <- Conformity(model, error_model, confidence=0.80,data.new=dataset$x.holdout)
@
...
CEC(abs(errors), confi_intervals)

# using spearman rank correlation insted to compare methods (not sure if this is the right way to compare
# or if there is a good reason the other authors chose CEC)
cor(abs(errors), sigmas, method="spearman")
cor(abs(errors), confi_intervals, method="spearman")
@
